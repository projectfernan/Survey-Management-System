...
class cSalesSurRep extends Controller {
    
    public static function cfShwSalesDetaRep(){
        if (Session::has('Desig') && Session::has('User') && Session::has('Name')){
            if(Session::get('Reports') == 1){
                return View::make('vSalesDetaRep');
            }
            else{
    ...
    public static function cfShwSalesBarRep(){
        if (Session::has('Desig') && Session::has('User') && Session::has('Name')){
            if(Session::get('Reports') == 1){
                return View::make('vSalesBarRep');
            }
            else{
                return Response::view('404', "Page Not Found.", 404);
            }
    ...
    public function cfGetSalesDetaRep(){
        $ajx = Input::get('data');
//        $base64 = base64_decode($ajx);
    ...
    public static function DetaRep($categ,$param,$fpdf){
        switch ($categ) {
                case "YesNo":
                    $rs = mSalesSurRep::mfSalesDetaRep($param);
                    ...
                    break;
                case "Comment":
                    $rs = mSalesSurRep::mfSalesCommRep($param);
                ...
                case "PoorAverageExcellent":
                    $rs = mSalesSurRep::mfSalesPaeRep($param);
                    ...
                    break;
                case "ExcellentPoor":
                    $rs = mSalesSurRep::mfSalesEPRep($param);
                ...
                case "OverallSatisfaction":
                    $rs = mSalesSurRep::mfSalesOSRep($param);
                ...
                case "NetPromoterScore":
                    $rs = mSalesSurRep::mfSalesNPSRep($param);
...
//                    }
    ...
    public function cfSalesDetaRep(){
        ...
        if(Session::has('RepCateg') && Session::has('DtFrm') && Session::has('DtTo')){
            ...
            $fpdf->Cell(200,5,'SALES SURVEY DETAILED REPORT');
            $fpdf->Ln(8);
            ...
            if($RepCateg <> ''){
                self::DetaRep($RepCateg,$param,$fpdf);
            }
            
            if($RepCateg2 <> ''){
                self::DetaRep($RepCateg2,$param,$fpdf);
            }
            
            if($RepCateg3 <> ''){
                self::DetaRep($RepCateg3,$param,$fpdf);
            }
            
            if($RepCateg4 <> ''){
                self::DetaRep($RepCateg4,$param,$fpdf);
            }
...
//        $base64 = base64_decode($ajx);
        ...
        switch ($js['_RepCateg']) {
            case "YesNo":
                $rs = mSalesSurRep::mfSalesBarRep($param);
